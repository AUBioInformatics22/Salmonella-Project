...
# go into the directory where all the FASTA files are
cd fastq_files

# where is the jar file?
jar_path=`echo $PATH | awk -F: '{print $1}'`
echo "jar file is in this directory: $jar_path"

# to run the script type in the commandline:
...
usage ()
{
  echo -e "${green}Usage: source trimmomatic.sh [*.extension]\n \
      extension: <fq> or <fastq> or <fq.gz> or <fastq.gz>\n \
      example: source trimmomatic.sh *.fq.gz\n ${reset}\n\
${yellow}Help:  source trimmomatic.sh -h or --help${reset}"
  return
}
        ...
        if [ -f $i ] ;then
		   ...
		   if [[ $count%2 -ne 0 ]]; then
		       ...
		       R2_unpair=${sample_name}_R2_unpair${extension}
                       ...
                       date && time java -jar \
		       $jar_path/trimmomatic-0.39.jar PE \
                       -threads 10 \

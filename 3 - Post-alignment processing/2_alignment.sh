...
date %c > ${dateum}_bwa_stats.txt
stat=${dateum}_bwa_stats.txt

        for sample in ${sample_list[@]}; do
            echo ${sample} >>$stat
            cat ${sample}.coverage_summary.txt >>$stat
            echo ' ' >> $stat
            cat ${sample}.merged.sorted.flagstat >>$stat
            echo '##################' >>$stat
            echo ' ' >>$stat
            echo '##################' >>$stat
            echo '##################' >>$stat
...
#Clean-up files
           ...
           mkdir merged_sorted_bam_files
           cp ./SRR107407??.merged.sorted.bam ./merged_sorted_bam_files/
